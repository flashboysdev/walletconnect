...
const url = 'https://bridge.bondex.io/'
...
async function getBalance() {
  if (web3.eth.defaultAccount === null)
    throw new Error(
      'Default account has not been set. Cannot get the balance of the empty account.'
    )
...
function showQrCode(data) {
  ...
  let deepLink = document.getElementById('deeplinking')
  deepLink.href = `example://test?${data.sessionId}?${data.sessionId}?${
    data.dappName
  }?${data.socketUrl}`
  const typeNumber = 0
...
async function requestNewSignedTransction(to, amount) {
  ...
  socket.emit('request-signed-transaction', {
    sessionId: sessionId,
    data: encrypt(JSON.stringify(tx))
  })
}
...
function requestNewAddress(chain) {
  socket.emit('request-new-address', {
    sessionId: sessionId,
    data: encrypt(chain)
  })
}
